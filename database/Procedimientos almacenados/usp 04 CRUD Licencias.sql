...
-- VERIFICACION FINAL 
SELECT * FROM CorpPiscis.licencia;
...
-- NUEVO REGISTRO DE DATOS
	...
	DELETE FROM CorpPiscis.contador WHERE Tabla = 'Licencias';
	INSERT INTO CorpPiscis.contador Values ('Licencias', 39);

--------------------------------------------------------------------------------------
-- PROCEDIMIENTO ALMACENADO PARA MOSTRAR CONTRATOS
DROP PROCEDURE IF EXISTS usp_mostrar_licencias$$
delimiter $$
create procedure usp_mostrar_licencias()
begin
	SELECT codLicencia, numLicencia, l.categoria, CONCAT(apellidos, ' ', nombres) as Trabajador, nombre,fechaExpedicion, fechaRevalidacion
	FROM licencia l 
